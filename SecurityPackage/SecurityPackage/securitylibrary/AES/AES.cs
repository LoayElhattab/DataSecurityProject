...
namespace SecurityLibrary.AES
{
    ...
    public class AES : CryptographicTechnique
    {
        ...
        {
            byte[,] stateCipherText = BlockToState(cipherText);
            ...
            stateCipherText = AddRoundKey(stateCipherText, keys[9]);
            stateCipherText = InvShiftRows(stateCipherText);
            stateCipherText = InvSubBytes(stateCipherText);

            for (int i = 8; i >= 0; i--)
            {
                stateCipherText = AddRoundKey(stateCipherText, keys[i]);
                stateCipherText = InvMixColumns(stateCipherText);
                stateCipherText = InvShiftRows(stateCipherText);
                stateCipherText = InvSubBytes(stateCipherText);
            }

            stateCipherText = AddRoundKey(stateCipherText, stateKey);

            string plainText = StateToBlock(stateCipherText);
            return plainText;
        }
        ...
        public byte[,] InvShiftRows(byte[,] state)
        {
            ...
            byte temp1 = state[1, 3];
            state[1, 3] = state[1, 2];
            state[1, 2] = state[1, 1];
            state[1, 1] = state[1, 0];
            state[1, 0] = temp1;

            byte temp21 = state[2, 2];
            byte temp22 = state[2, 3];
            state[2, 2] = state[2, 0];
            state[2, 3] = state[2, 1];
            state[2, 0] = temp21;
            state[2, 1] = temp22;

            byte temp3 = state[3, 0];
            state[3, 0] = state[3, 1];
            state[3, 1] = state[3, 2];
            state[3, 2] = state[3, 3];
            state[3, 3] = temp3;
        ...
        public byte[,] InvMixColumns(byte[,] state)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                newState[0, i] = (byte)(GFMultiply(state[0, i], 0x0E) ^ GFMultiply(state[1, i], 0x0B) ^
                                         GFMultiply(state[2, i], 0x0D) ^ GFMultiply(state[3, i], 0x09));
                newState[1, i] = (byte)(GFMultiply(state[0, i], 0x09) ^ GFMultiply(state[1, i], 0x0E) ^
                                         GFMultiply(state[2, i], 0x0B) ^ GFMultiply(state[3, i], 0x0D));
                newState[2, i] = (byte)(GFMultiply(state[0, i], 0x0D) ^ GFMultiply(state[1, i], 0x09) ^
                                         GFMultiply(state[2, i], 0x0E) ^ GFMultiply(state[3, i], 0x0B));
                newState[3, i] = (byte)(GFMultiply(state[0, i], 0x0B) ^ GFMultiply(state[1, i], 0x0D) ^
                                         GFMultiply(state[2, i], 0x09) ^ GFMultiply(state[3, i], 0x0E));
            }
        ...
        public byte GFMultiply(byte a, byte value)
        {
            if (value == 0x01)
                return a;
            else if (value == 0x02 || value == 0x03)
            {
                if (value == 0x02) return (byte)((a << 1) ^ (a >= 0x80 ? 0x1B : 0x00));
            ...
            else
            {
                ...
                if ((value & 0x01) != 0) result ^= temp;
                temp = (byte)((temp << 1) ^ (temp >= 0x80 ? 0x1B : 0x00));

                if ((value & 0x02) != 0) result ^= temp;
                temp = (byte)((temp << 1) ^ (temp >= 0x80 ? 0x1B : 0x00));

                if ((value & 0x04) != 0) result ^= temp;
                temp = (byte)((temp << 1) ^ (temp >= 0x80 ? 0x1B : 0x00));

                if ((value & 0x08) != 0) result ^= temp;

                return result;
            }
        }

        // Inverse S-box table - this would need to be defined with the actual values
        readonly byte[,] invSBox = new byte[16, 16] {
